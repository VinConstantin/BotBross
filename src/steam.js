                ...
                req.end(function (result) {
                    ...
                    if(string.response.total_count > 2){
                        if(parameter == "played"){
                            ...
                            var msg = "**__" + name + "__**\n\n";
                            for(var i = 0; i < 3; i++){
                                msg += "**" + string.response.games[i].name + "**```Last 2 weeks: " +
                                Number(string.response.games[i].playtime_2weeks / 60).toFixed(2) + "Hrs\nAll time: " + 
                                Number(string.response.games[i].playtime_forever/60).toFixed(2) + "Hrs```";
                            }
                            cb(msg);
                        } else {
                            cb("Invalid entry");
